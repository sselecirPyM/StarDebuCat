...
public abstract class ReadOnlyKDTree2<T>
{
    public abstract void ForRange(Vector2 position, float radius, Action<T> action);
    public abstract void ForRange(Vector2 min, Vector2 max, Action<T> action);
    public abstract void Nearest(Vector2 position, float maxDistance, Predicate<T> filter, Action<T> action);
}
...
public sealed class KDTree2<T> : ReadOnlyKDTree2<T>
{
    struct BB
    {
        public Vector2 min;
    ...
    public override void ForRange(Vector2 position, float radius, Action<T> action)
    {
        _Search(root, position, radius, radius * radius, action);
    }

    public override void ForRange(Vector2 min, Vector2 max, Action<T> action)
    {
        _Search(root, new BB() { min = Vector2.Min(min, max), max = Vector2.Max(min, max), }, action);
    }

    public override void Nearest(Vector2 position, float maxDistance, Predicate<T> filter, Action<T> action)
    {
        int resultIndex = -1;
    ...
    Tree2Node _ParallelBuild(int left, int right, int deep, in BB bb)
    {
        if (right - left < 12 || deep > 8)
        {
            var child = new Tree2Node()
    ...
    Tree2Node _ParallelAddChild(int left, int right, int m, float midValue, int deep, SplitType splitType, in BB bb)
    {
        BB bb0 = bb;
        BB bb1 = bb;
        switch (splitType)
