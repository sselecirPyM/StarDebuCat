...
namespace MilkWangBase
{
    public class AnalysisSystem
    {
        ...
        public AutoCurve CollectedMinerals = new();
        public AutoCurve CollectedVespene = new();
        public AutoCurve KilledMineralsArmy = new();
        public AutoCurve KilledMineralsTechnology = new();
        public AutoCurve KilledVespeneArmy = new();
        public AutoCurve KilledVespeneTechnology = new();

        public AutoCurve TotalUsedMineralsArmy = new();
        public AutoCurve TotalUsedMineralsTechnology = new();
        public AutoCurve TotalUsedVespeneArmy = new();
        public AutoCurve TotalUsedVespeneTechnology = new();

        public AutoCurve UsedMineralsArmy = new();
        public AutoCurve UsedVespeneArmy = new();

        public AutoCurve LostMineralsArmy = new();
        public AutoCurve LostVespeneArmy = new();
        public AutoCurve SpentMinerals = new();
        public AutoCurve SpentVespene = new();
        public AutoCurve FoodUsedArmy = new();

        public AutoCurve FoodUsedX = new();
        public AutoCurve FoodCapX = new();
        ...
        void CollectScores()
        {
            var scoreDetails = inputSystem.observation.Observation.Score.ScoreDetails;

            CollectionRateMinerals.Set(scoreDetails.CollectionRateMinerals);
            CollectionRateVespene.Set(scoreDetails.CollectionRateVespene);
            CollectedMinerals.Set(scoreDetails.CollectedMinerals);
            CollectedVespene.Set(scoreDetails.CollectedVespene);
            KilledMineralsArmy.Set(scoreDetails.KilledMinerals.Army);
            KilledMineralsTechnology.Set(scoreDetails.KilledMinerals.Technology);
            KilledVespeneArmy.Set(scoreDetails.KilledVespene.Army);
            KilledVespeneTechnology.Set(scoreDetails.KilledVespene.Technology);
            TotalUsedMineralsArmy.Set(scoreDetails.TotalUsedMinerals.Army);
            TotalUsedMineralsTechnology.Set(scoreDetails.TotalUsedMinerals.Technology);
            TotalUsedVespeneArmy.Set(scoreDetails.TotalUsedVespene.Army);
            TotalUsedVespeneTechnology.Set(scoreDetails.TotalUsedVespene.Technology);
            LostMineralsArmy.Set(scoreDetails.LostMinerals.Army);
            LostVespeneArmy.Set(scoreDetails.LostVespene.Army);
            SpentMinerals.Set(scoreDetails.SpentMinerals);
            SpentVespene.Set(scoreDetails.SpentVespene);
            UsedMineralsArmy.Set(scoreDetails.UsedMinerals.Army);
            UsedVespeneArmy.Set(scoreDetails.UsedVespene.Army);
            FoodUsedArmy.Set(scoreDetails.FoodUsed.Army);

            var playerCommon = inputSystem.observation.Observation.PlayerCommon;
            FoodCapX.Set(playerCommon.FoodCap);
            FoodUsedX.Set(playerCommon.FoodUsed);

            GameLoop = inputSystem.observation.Observation.GameLoop;
        ...
        void _CollectUnits(List<Unit> units1, object[] conditions)
        {
            units1.Clear();
            var xunits = Optimise(conditions, out var equal);
            if (equal)
                units1.AddRange(xunits);
            else
            {
                foreach (var unit in xunits)
                {
